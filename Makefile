...
.PHONY: main_MPI
main_MPI: main.c pbm/funcs.c pbm/dft.c
	test -d "$(BUILD_DIR)" || mkdir $(BUILD_DIR)
	test -d "$(IMAGES_DIR)" || mkdir $(IMAGES_DIR)
	$(CC_MPI) pbm/funcs.c pbm/dft.c main.c $(CFLAGS) -o $(BUILD_DIR)/filter_image
...
deploy:
	...
	scp -r $(DEPLOY_DIR)/* $(USER)@orion2.isr.uc.pt:$(USER_DIR)/
	rm -r $(DEPLOY_DIR)

download_image:
	test -d "$(DOWNLOAD_DIR)" || mkdir $(DOWNLOAD_DIR)
	scp -r $(USER)@orion2.isr.uc.pt:$(USER_DIR)/build/images/* $(DOWNLOAD_DIR)
